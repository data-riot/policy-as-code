...
class TimeConfiguration:
    ...
    def __post_init__(self):
        ...
        if self.allowed_timezones is None:
            object.__setattr__(self, 'allowed_timezones', ["UTC", "Europe/Helsinki", "America/New_York"])
...
class DeterministicTimeManager:
    ...
    def get_current_time(self, source: Optional[TimeSource] = None) -> DeterministicTimestamp:
        ...
        if source == TimeSource.FIXED_TIME:
            ...
            raise DecisionLayerError("invalid_time_source", f"Unknown time source: {source}")
    ...
    def _get_normalized_utc_time(self) -> DeterministicTimestamp:
        ...
        if clock_skew_ms and abs(clock_skew_ms) > 0:
            if self.config.skew_strategy == ClockSkewStrategy.CORRECT:
                ...
                skew_corrected = True
            elif self.config.skew_strategy == ClockSkewStrategy.REJECT and abs(clock_skew_ms) > self.config.max_skew_ms:
                raise DecisionLayerError("clock_skew_exceeded", f"Clock skew {clock_skew_ms}ms exceeds threshold {self.config.max_skew_ms}ms")
            else:
    ...
    def get_time_range(self, start_time: datetime, end_time: datetime) -> List[DeterministicTimestamp]:
...
class TimeNormalizer:
    ...
    @staticmethod
    def normalize_to_utc(timestamp: Union[datetime, str], source_timezone: Optional[str] = None) -> datetime:
        """Normalize any timestamp to UTC"""
        if isinstance(timestamp, str):
            try:
                timestamp = datetime.fromisoformat(timestamp)
            except ValueError:
                raise DecisionLayerError("invalid_timestamp_format", f"Cannot parse timestamp: {timestamp}")
    ...
    @staticmethod
    def is_within_tolerance(actual: datetime, expected: datetime, tolerance_ms: int = 1000) -> bool:
...
def create_time_manager(config: TimeConfiguration = PRODUCTION_TIME_CONFIG) -> DeterministicTimeManager:
