...
class AuditWorker:
    ...
    async def _verify_trace_hash(self, trace: TraceRecord) -> AuditResult:
        ...
        try:
            ...
            trace_data = {
                ...
                "input_hash": hashlib.sha256(json.dumps(trace.input_json, sort_keys=True).encode()).hexdigest(),
                "output_hash": hashlib.sha256(json.dumps(trace.output_json, sort_keys=True).encode()).hexdigest(),
            }
            
            computed_hash = hashlib.sha256(json.dumps(trace_data, sort_keys=True).encode()).hexdigest()
            ...
            if computed_hash == trace.df_hash:
                ...
                return AuditResult(
                    ...
                    details={"trace_id": trace.trace_id, "expected": trace.df_hash, "computed": computed_hash}
                )
        except Exception as e:
            return AuditResult(
                check_type=IntegrityCheck.HASH_VERIFICATION,
        ...
        try:
            ...
            if not release:
                return AuditResult(
                    ...
                    details={"trace_id": trace.trace_id, "df_id": trace.df_id, "version": trace.version}
                )
    ...
    async def _verify_signatures(self, trace: TraceRecord) -> AuditResult:
        ...
        try:
            ...
            reviewer_signed = any(sig.role.value == "reviewer" for sig in release.signatures)
            ...
            if owner_signed and reviewer_signed:
                return AuditResult(
                    ...
                    details={"trace_id": trace.trace_id, "signature_count": len(release.signatures)}
                )
            else:
                return AuditResult(
                    check_type=IntegrityCheck.SIGNATURE_VERIFICATION,
                    ...
                    details={"trace_id": trace.trace_id, "owner_signed": owner_signed, "reviewer_signed": reviewer_signed}
                )
        except Exception as e:
            return AuditResult(
                check_type=IntegrityCheck.SIGNATURE_VERIFICATION,
        ...
        try:
            ...
            if release.legal_references:
                return AuditResult(
                    ...
                    details={"trace_id": trace.trace_id, "legal_ref_count": len(release.legal_references)}
                )
            else:
                return AuditResult(
                    check_type=IntegrityCheck.LEGAL_COMPLIANCE,
    ...
    async def _detect_drift(self, traces: List[TraceRecord]) -> List[AuditResult]:
        ...
        try:
            ...
            for function_key, function_trace_list in function_traces.items():
                ...
                drift_result = await self._analyze_function_drift(function_key, function_trace_list)
    ...
    async def _analyze_function_drift(self, function_key: str, traces: List[TraceRecord]) -> AuditResult:
        ...
        try:
            ...
            if len(decision_keys) > 5:  # Arbitrary threshold
                return AuditResult(
                    ...
                    details={"function_key": function_key, "unique_keys": len(decision_keys)}
                )
            else:
                return AuditResult(
                    check_type=IntegrityCheck.HASH_VERIFICATION,  # Using as generic check type
    ...
    def _generate_summary(self, report: IntegrityReport) -> str:
        ...
        success_rate = ((report.checked_traces - report.failed_checks) / report.checked_traces * 100) if report.checked_traces > 0 else 100
