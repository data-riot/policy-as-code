...
class PostgreSQLTraceWriter(TraceWriter):
    ...
    def _compute_expected_chain_hash(
        self, trace_id: str, prev_hash: Optional[str]
    ) -> str:
...
class PostgreSQLTraceReader(TraceReader):
    ...
    async def get_traces_by_function(
        ...
        try:
            ...
            raise LedgerError(
                "get_traces_by_function", f"Failed to get traces: {str(e)}"
            )
    ...
    async def get_traces_by_timeframe(
        ...
        if not self.pool:
            raise LedgerError(
                "get_traces_by_timeframe", "Database pool not initialized"
            )
        ...
        try:
            ...
            raise LedgerError(
                "get_traces_by_timeframe", f"Failed to get traces: {str(e)}"
            )
...
def create_postgresql_ledger(
    connection_string: str, table_name: str = "decision_trace"
):
