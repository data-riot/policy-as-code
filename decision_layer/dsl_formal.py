...
class DSLGrammar:
    ...
    def _validate_rule_syntax(rule: Dict[str, Any], index: int) -> List[str]:
        ...
        required_rule_fields = ["rule_id", "rule_type", "priority", "conditions", "actions", "description"]
        ...
        if "conditions" in rule:
            for j, condition in enumerate(rule["conditions"]):
                condition_errors = DSLGrammar._validate_condition_syntax(condition, index, j)
    ...
    @staticmethod
    def _validate_condition_syntax(condition: Dict[str, Any], rule_index: int, condition_index: int) -> List[str]:
        ...
        for field in required_fields:
            if field not in condition:
                errors.append(f"Rule {rule_index}, Condition {condition_index}: Missing field: {field}")
        
        # Validate operator
        if "operator" in condition:
            valid_operators = [op.value for op in Operator]
            if condition["operator"] not in valid_operators:
                errors.append(f"Rule {rule_index}, Condition {condition_index}: Invalid operator: {condition['operator']}")
        
        return errors
    
    @staticmethod
    def _validate_action_syntax(action: Dict[str, Any], rule_index: int, action_index: int) -> List[str]:
        ...
        for field in required_fields:
            if field not in action:
                errors.append(f"Rule {rule_index}, Action {action_index}: Missing field: {field}")
...
class RuleConflictDetector:
            ...
            if i > 0:  # Skip first rule (highest priority)
                ...
                if self._rule_is_shadowed(rule, higher_priority_rules):
                    conflict = RuleConflict(
                        ...
                        severity="warning",
                        details={"shadowing_rules": [r.rule_id for r in higher_priority_rules]}
                    )
                    self.conflicts.append(conflict)
    ...
    def _detect_circular_dependencies(self, rules: List[DSLRule]) -> None:
        ...
        for rule in rules:
            for action in rule.actions:
                if action.action_type == "set" and action.field in [c.field for c in rule.conditions]:
    ...
    def _detect_type_mismatches(self, rules: List[DSLRule], input_schema: Dict[str, Any]) -> None:
        """Detect type mismatches between rules and schema"""
        for rule in rules:
            for condition in rule.conditions:
                field_type = self._get_field_type(condition.field, input_schema)
                if field_type and not self._is_type_compatible(condition.value, field_type, condition.operator):
                    conflict = RuleConflict(
                        ...
                        severity="error",
                        details={"field": condition.field, "expected_type": field_type, "actual_value": condition.value}
                    )
                    self.conflicts.append(conflict)
    
    def _detect_missing_fields(self, rules: List[DSLRule], input_schema: Dict[str, Any]) -> None:
    ...
    def _rule_is_shadowed(self, rule: DSLRule, higher_priority_rules: List[DSLRule]) -> bool:
    ...
    def _is_type_compatible(self, value: Any, expected_type: str, operator: Operator) -> bool:
...
class DSLStaticAnalyzer:
    ...
    def _generate_warnings(self, schema: DSLSchema) -> List[str]:
        ...
        for rule in schema.rules:
            if not rule.conditions:
                warnings.append(f"Rule {rule.rule_id} has no conditions (always matches)")
    ...
    def _calculate_metrics(self, schema: DSLSchema) -> Dict[str, Any]:
        ...
        return {
            ...
            "rule_types": {t.value: len([r for r in schema.rules if r.rule_type == t]) for t in RuleType},
            "priority_range": {
            ...
            "average_conditions_per_rule": sum(len(r.conditions) for r in schema.rules) / max(len(schema.rules), 1),
            "average_actions_per_rule": sum(len(r.actions) for r in schema.rules) / max(len(schema.rules), 1),
        }
    
    def _generate_recommendations(self, schema: DSLSchema, conflicts: List[RuleConflict]) -> List[str]:
        ...
        if any(c.conflict_type == ConflictType.UNREACHABLE_RULE for c in conflicts):
            recommendations.append("Review rule priorities to ensure all rules are reachable")
        
        # General recommendations
        if len(schema.rules) > 20:
            recommendations.append("Consider splitting large rule sets into multiple schemas")
    ...
    "rules": [
        {
            ...
            "actions": [
                {"action_type": "set", "field": "approved", "value": True}
            ],
        ...
        },
        {
            ...
            "actions": [
                {"action_type": "set", "field": "approved", "value": False}
            ],
