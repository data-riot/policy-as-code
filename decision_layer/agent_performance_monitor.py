...
class AgentPerformanceMonitor:
    ...
    async def record_performance_metric(
        ...
        try:
            ...
            logger.debug(f"Recorded {metric_type.value} metric for agent {agent_id}: {value}")
    ...
    async def get_agent_effectiveness(self, agent_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            effectiveness = await self._calculate_effectiveness_metrics(agent_id, recent_data)
            
            # Get trend analysis
            trends = await self._analyze_performance_trends(agent_id, recent_data)
            
            # Get recommendations
            recommendations = await self._generate_effectiveness_recommendations(agent_id, effectiveness, trends)
    ...
    async def detect_agent_drift(self, agent_id: str) -> List[DriftDetectionResult]:
        ...
        try:
            ...
            performance_drift = await self._detect_performance_drift(agent_id, agent_data)
    ...
    async def optimize_agent_coordination(self, coordination_data: Dict[str, Any]) -> List[OptimizationRecommendation]:
        """
        ...
        try:
            ...
            response = await self.llm_integration._call_llm(prompt, ReasoningMode.AUTONOMOUS)
    ...
    async def generate_performance_report(self, agent_id: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            report["recommendations"] = await self._generate_global_recommendations(agents)
    ...
    async def _update_agent_profile(self, agent_id: str):
        ...
        try:
            ...
            for metric_type in PerformanceMetric:
                metric_data = [dp.value for dp in recent_data if dp.metric_type == metric_type]
                if metric_data:
    ...
    async def _calculate_effectiveness_metrics(self, agent_id: str, data: List[PerformanceDataPoint]) -> Dict[str, Any]:
        ...
        for metric_type in PerformanceMetric:
            ...
            if metric_data:
                metrics[metric_type.value] = {
                    ...
                    "std_dev": statistics.stdev(metric_data) if len(metric_data) > 1 else 0,
                    "min": min(metric_data),
                    "max": max(metric_data),
    ...
    async def _analyze_performance_trends(self, agent_id: str, data: List[PerformanceDataPoint]) -> Dict[str, str]:
        """Analyze performance trends for an agent"""
        trends = {}
        
        for metric_type in PerformanceMetric:
            metric_data = [(dp.timestamp, dp.value) for dp in data if dp.metric_type == metric_type]
        ...
        for metric, data in effectiveness.items():
            if isinstance(data, dict) and "mean" in data:
                if data["mean"] < 0.7:  # Threshold for low performance
                    recommendations.append(f"Improve {metric} (current: {data['mean']:.2f})")
        
        return recommendations
    
    async def _detect_performance_drift(self, agent_id: str, data: List[PerformanceDataPoint]) -> Optional[DriftDetectionResult]:
        ...
        for metric_type in PerformanceMetric:
            ...
            if recent_avg < earlier_avg * 0.8:  # 20% decline threshold
                return DriftDetectionResult(
                    ...
                    recommended_actions=["Review recent changes", "Check resource allocation", "Monitor for errors"],
                    detected_at=datetime.now(timezone.utc)
                )
        
        return None
    
    async def _detect_behavioral_drift(self, agent_id: str, data: List[PerformanceDataPoint]) -> Optional[DriftDetectionResult]:
        """Detect behavioral drift in agent"""
        # Mock implementation - would analyze decision patterns, response styles, etc.
        return None
    
    async def _detect_decision_bias_drift(self, agent_id: str, data: List[PerformanceDataPoint]) -> Optional[DriftDetectionResult]:
        """Detect decision bias drift"""
        # Mock implementation - would analyze decision patterns for bias
        return None
    
    async def _calculate_performance_score(self, profile: AgentPerformanceProfile) -> float:
    ...
    async def _generate_overall_insights(self, agents: List[str]) -> List[str]:
        ...
        total_score = sum(
            self.agent_profiles.get(agent, AgentPerformanceProfile("", {}, {}, {}, 0.0, datetime.now(timezone.utc))).performance_score
            for agent in agents
        )
        ...
        if avg_score > 0.8:
            ...
            insights.append("Overall agent performance is good with room for improvement")
        else:
        ...
        declining_agents = [
            ...
            and any(trend == "declining" for trend in self.agent_profiles[agent].trend_analysis.values())
        ]
...
def create_agent_performance_monitor(llm_integration: LLMIntegration) -> AgentPerformanceMonitor:
