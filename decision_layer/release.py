...
class Release:
    ...
    def add_signature(self, signature: DigitalSignature) -> None:
        ...
        for existing_sig in self.signatures:
            if existing_sig.signer_id == signature.signer_id and existing_sig.role == signature.role:
                raise ReleaseError(
                    "duplicate_signature",
    ...
    def _update_status(self) -> None:
        ...
        reviewer_signed = any(sig.role == SignerRole.REVIEWER for sig in self.signatures)
    ...
    def can_activate(self) -> bool:
        ...
        return (
            self.status == ReleaseStatus.APPROVED and
            len(self.legal_references) > 0 and
            any(sig.role == SignerRole.OWNER for sig in self.signatures) and
            any(sig.role == SignerRole.REVIEWER for sig in self.signatures)
        )
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Release":
        """Create from dictionary"""
        legal_refs = [LawReference.from_dict(ref) for ref in data.get("legal_references", [])]
        signatures = [DigitalSignature.from_dict(sig) for sig in data.get("signatures", [])]
...
class SignatureValidator:
    ...
    def validate_signature_requirements(release: Release) -> List[str]:
        ...
        reviewer_signed = any(sig.role == SignerRole.REVIEWER for sig in release.signatures)
        ...
        if len(signers) < 2:
            errors.append("Separation of duties requires at least two different signers")
...
class ReleaseManager:
    ...
    def __init__(self):
        self.releases: Dict[str, Dict[str, Release]] = {}  # function_id -> version -> Release
    ...
    def create_release(
        ...
        if function_id in self.releases and version in self.releases[function_id]:
            raise ReleaseError(
                "duplicate_release",
                f"Release {function_id} v{version} already exists"
            )
        
        # Validate legal references
        validated_refs = LegalReferenceValidator.validate_reference_list(legal_references)
    ...
    def sign_release(
        ...
        if not release:
            raise ReleaseError(
                "release_not_found",
                f"Release {function_id} v{version} not found"
            )
    ...
    def activate_release(self, function_id: str, version: str) -> Release:
        ...
        if errors:
            raise ReleaseError(
                "activation_failed",
                f"Cannot activate release: {'; '.join(errors)}"
            )
