...
class ExplanationRenderer:
    ...
    async def explain_decision(self, trace_id: str, redact_sensitive: bool = True) -> DecisionExplanation:
        ...
        release = self.release_manager.get_release(trace_record.df_id, trace_record.version)
        if not release:
            raise ExplanationError(
                "release_not_found",
        ...
        explanation = await self._generate_explanation(trace_record, release, redact_sensitive)
        return explanation
    ...
    async def _generate_explanation(
        ...
        output_summary = self._summarize_output(trace_record.output_json, redact_sensitive)
        
        # Determine redacted fields
        redacted_fields = self._get_redacted_fields(trace_record.input_json, redact_sensitive)
    ...
    def _generate_decision_path(self, trace_record: TraceRecord) -> List[str]:
        ...
        path.append(f"Applied decision function: {trace_record.df_id} v{trace_record.version}")
    ...
    def _summarize_input(self, input_data: Dict[str, Any], redact_sensitive: bool) -> Dict[str, Any]:
    ...
    def _summarize_output(self, output_data: Dict[str, Any], redact_sensitive: bool) -> Dict[str, Any]:
        """Summarize output data for explanation"""
        summary = {}
        
        for key, value in output_data.items():
            if redact_sensitive and key.lower() in {"internal_notes", "debug_info", "trace"}:
                summary[key] = "[REDACTED]"
            elif isinstance(value, (str, int, float, bool)):
                summary[key] = value
            elif isinstance(value, dict):
    ...
    def _get_redacted_fields(self, input_data: Dict[str, Any], redact_sensitive: bool) -> List[str]:
...
class ExplanationAPI:
    ...
    async def get_explanation(self, trace_id: str, redact_sensitive: bool = True) -> DecisionExplanation:
    ...
    async def get_explanations_for_function(
        ...
        for trace in traces:
            try:
                explanation = await self.renderer.explain_decision(trace.trace_id, redact_sensitive)
...
def create_explanation_api(release_manager: ReleaseManager, trace_ledger: TraceLedger) -> ExplanationAPI:
