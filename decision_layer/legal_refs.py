...
class LawReference:
    ...
    def _is_valid_eurlex_uri(self, uri: str) -> bool:
        ...
        eurlex_pattern = r'^https://eur-lex\.europa\.eu/legal-content/EN/TXT/\?uri=CELEX:\d+[A-Z]?$'
...
class LegalReferenceValidator:
    ...
    @staticmethod
    def validate_reference(reference: Union[LawReference, Dict[str, Any]]) -> LawReference:
        """Validate and normalize a legal reference"""
        if isinstance(reference, dict):
            try:
                return LawReference.from_dict(reference)
            except Exception as e:
                raise LegalReferenceError("validation", f"Invalid reference format: {str(e)}")
        elif isinstance(reference, LawReference):
            return reference
        else:
            raise LegalReferenceError("validation", f"Invalid reference type: {type(reference)}")
    
    @staticmethod
    def validate_reference_list(references: List[Union[LawReference, Dict[str, Any]]]) -> List[LawReference]:
    ...
    @staticmethod
    def require_legal_references(references: List[Union[LawReference, Dict[str, Any]]]) -> List[LawReference]:
        ...
        if not validated:
            raise LegalReferenceError(
                "empty_reference", 
                "No valid legal references found"
            )
...
class LegalReferenceRegistry:
    ...
    def search_references(self, system: Optional[LegalSystem] = None, act_id: Optional[str] = None) -> List[LawReference]:
...
def validate_legal_references_in_metadata(metadata: Dict[str, Any]) -> List[LawReference]:
        ...
        if isinstance(ref, str):
            ...
            try:
                ...
                if "finlex.fi" in parsed.netloc:
                    ...
                    act_id = parsed.path.split('/')[-2] if parsed.path.endswith('/') else parsed.path.split('/')[-1]
                    legal_refs.append(LawReference(
                        system=LegalSystem.FINLEX,
                        act_id=act_id,
                        uri=ref
                    ))
