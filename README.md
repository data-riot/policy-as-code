...
decision_layer/
  ...
  executor.py         # Execution engine with schema validation
  registry.py         # Function registry with versioning
  trace_sink.py       # Trace logging infrastructure
  schemas.py          # Schema validation system
  shadow_runner.py    # Shadow testing and regression analysis
  dsl_loader.py       # YAML policy loader with schema support
...
policies/
  refund_policy.yaml      # Policy with schema

tests/
  test_features.py  # Comprehensive test suite
  data/
    sample_order.json        # Single test input
    test_inputs.json         # Multiple test inputs for shadow testing
```
