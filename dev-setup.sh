...
DATABASE_URL=postgresql://user:password@localhost:5432/decision_layer
...
run_quality_checks() {
    ...
    # Format code
    $PYTHON_CMD -m black decision_layer tests examples --check || {
        print_warning "Code formatting issues found. Run 'black decision_layer tests examples' to fix"
    }

    # Sort imports
    $PYTHON_CMD -m isort decision_layer tests examples --check-only || {
        print_warning "Import sorting issues found. Run 'isort decision_layer tests examples' to fix"
    }

    # Lint code
    $PYTHON_CMD -m flake8 decision_layer tests examples || {
        print_warning "Linting issues found"
    }

    # Type checking
    $PYTHON_CMD -m mypy decision_layer || {
...
python -m uvicorn decision_layer.api:app --host ${API_HOST:-localhost} --port ${API_PORT:-8000} --reload
EOF
...
python -m pytest tests/ --cov=decision_layer --cov-report=html --cov-report=term-missing
...
show_completion() {
    ...
    echo ""
    echo "5. Format code:"
    echo "   black decision_layer tests examples"
    echo "   isort decision_layer tests examples"
    echo ""
    echo "6. Check code quality:"
    echo "   flake8 decision_layer tests examples"
    echo "   mypy decision_layer"
    echo ""
    ...
    echo "  python -m mypy decision_layer       # Type check"
