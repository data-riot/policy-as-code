...
async def get_engine_stats():
    ...
    try:
        ...
        avg_duration = (
            sum(t.get("duration", 0) for t in traces) / len(traces) if traces else 0
        )
...
def show_dashboard():
    ...
    st.markdown(
        '<h1 class="main-header">🎯 Decision Layer Dashboard</h1>',
        unsafe_allow_html=True,
    )
    ...
    with col1:
        ...
        if traces:
            ...
            fig = px.line(
                hourly_stats, x="hour", y="executions", title="Executions by Hour"
            )
    ...
    for trace in traces:
        with st.container():
            ...
            status_badge = (
                "success-badge" if trace["status"] == "success" else "error-badge"
            )
            status_icon = "✅" if trace["status"] == "success" else "❌"
            col3.markdown(
                f'<span class="{status_badge}">{status_icon} {trace["status"]}</span>',
                unsafe_allow_html=True,
            )
            ...
            with st.expander(f"🔧 {function_id}", expanded=False):
                try:
                    versions = asyncio.run(
                        st.session_state.engine.list_versions(function_id)
                    )
                    ...
                    with col1:
                        ...
                        function_traces = [
                            t for t in traces if t["function_id"] == function_id
                        ]
                        ...
                        if function_traces:
                            ...
                            avg_duration = sum(
                                t["duration"] for t in function_traces
                            ) / len(function_traces)
...
def show_function_editor():
    ...
    try:
        ...
        if selected_function:
            versions = asyncio.run(
                st.session_state.engine.list_versions(selected_function)
            )
            ...
            try:
                code = asyncio.run(
                    st.session_state.engine.storage.load_function(
                ...
                with col2:
                    ...
                    if st.button("🚀 Run Test", type="primary"):
                        try:
                            ...
                            # Execute function
                            result = asyncio.run(
                                st.session_state.engine.execute(
                    ...
                    if st.button("📋 Validate Code", type="secondary"):
                        try:
                            ...
                            if "def decision_function" not in new_code:
                                st.error(
                                    "Code must contain 'decision_function' definition"
                                )
                            else:
                ...
                with col2:
                    if st.button("🚀 Deploy", type="primary"):
                        try:
                            asyncio.run(
                                st.session_state.engine.deploy_function(
                                    selected_function, new_version, new_code
                                )
                            )
                            st.success(
                                f"Successfully deployed {selected_function} v{new_version}"
                            )
                            st.rerun()
                        except Exception as e:
...
def show_deploy():
    ...
    version = st.text_input(
        "Version:", value="v1.0", help="Version number for this function"
    )
...
def decision_function(input_data: Dict[str, Any], context: DecisionContext) -> Dict[str, Any]:
    ...
    with col1:
        test_input = st.text_area(
            "Test Input (JSON):",
            value='{"amount": 500, "customer_score": 750}',
            height=150,
        )
    ...
    if st.button("🚀 Deploy Function", type="primary", disabled=not function_id):
        if not function_id:
            ...
            try:
                asyncio.run(
                    st.session_state.engine.deploy_function(function_id, version, code)
                )
...
def show_traces():
    ...
    with col1:
        try:
            functions = asyncio.run(st.session_state.engine.list_functions())
            function_filter = st.selectbox("Function", ["All"] + functions)
        except:
            function_filter = "All"
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "status": st.column_config.SelectboxColumn(
                    "Status", options=["Success", "Error"]
                ),
                "duration": st.column_config.NumberColumn(
                    "Duration (ms)", format="%d ms"
                ),
            },
            hide_index=True,
...
def main():
    ...
    if not st.session_state.get("engine_ready", False):
        st.error(
            "Failed to initialize Decision Layer engine. Please check your configuration."
        )
        return
