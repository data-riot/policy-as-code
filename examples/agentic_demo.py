...
from decision_layer import (
    DecisionEngine, FunctionRegistry, create_decision_engine, create_function_registry,
    LLMIntegration, create_llm_integration, AgenticContext, ReasoningMode,
    ConversationalInterface, create_conversational_interface, ConversationChannel,
    WorkflowOrchestrator, create_workflow_orchestrator, WorkflowDefinition, WorkflowTask, TaskStatus,
    AgentPerformanceMonitor, create_agent_performance_monitor, PerformanceMetric
)
...
class AgenticDemo:
    ...
    async def step_1_setup_agentic_system(self):
        ...
        self.performance_monitor = create_agent_performance_monitor(self.llm_integration)
        
        # Register some agents
        self.workflow_orchestrator.register_agent("benefits_agent", ["benefits_processing", "eligibility_check"], "ai")
        self.workflow_orchestrator.register_agent("compliance_agent", ["legal_compliance", "audit"], "ai")
        self.workflow_orchestrator.register_agent("citizen_service_agent", ["citizen_interaction", "explanation"], "ai")
    ...
    async def step_2_demonstrate_llm_reasoning(self):
        ...
        def loan_approval_function(input_data: Dict[str, Any], context) -> Dict[str, Any]:
            ...
            if credit_score >= 700 and income >= 50000 and amount <= 100000:
                return {"approved": True, "interest_rate": 3.5, "reason": "Excellent credit profile"}
            elif credit_score >= 650 and income >= 40000 and amount <= 75000:
                return {"approved": True, "interest_rate": 4.5, "reason": "Good credit profile"}
            else:
                return {"approved": False, "reason": "Does not meet minimum requirements"}
    ...
    async def step_4_demonstrate_workflow_orchestration(self):
        ...
        print(f"   - Estimated Duration: {sum(task.estimated_duration for task in workflow_tasks)} minutes")
        
        # Start workflow execution
        execution_id = await self.workflow_orchestrator.start_workflow(
            workflow_definition=workflow_definition,
            initial_context={"citizen_id": "CITIZEN_005", "application_type": "unemployment_benefits"}
        )
        ...
        for i in range(5):  # Check 5 times
            ...
            print(f"   Status Check {i+1}: {status['status']} - {status['progress']['completed_tasks']}/{status['progress']['total_tasks']} tasks")
    ...
    async def step_5_demonstrate_performance_monitoring(self):
        ...
        agents = ["loan_agent", "conversational_agent", "benefits_agent", "compliance_agent"]
        metrics = [PerformanceMetric.DECISION_ACCURACY, PerformanceMetric.RESPONSE_TIME, 
                  PerformanceMetric.CITIZEN_SATISFACTION, PerformanceMetric.ERROR_RATE]
        
        print("   Recording performance data...")
        for agent in agents:
            for metric in metrics:
                # Simulate some performance data
                value = 0.7 + (hash(agent + metric.value) % 30) / 100  # Random value between 0.7-1.0
                await self.performance_monitor.record_performance_metric(
                    agent_id=agent,
                    metric_type=metric,
                    value=value,
                    context={"demo": True, "timestamp": datetime.now(timezone.utc).isoformat()}
                )
        
        # Get performance reports
        print("\n   Performance Analysis:")
        for agent in agents:
            effectiveness = await self.performance_monitor.get_agent_effectiveness(agent)
            if "error" not in effectiveness:
                print(f"   {agent}:")
                print(f"     Performance Score: {effectiveness.get('effectiveness_metrics', {}).get('decision_accuracy', {}).get('mean', 0):.2f}")
                print(f"     Data Points: {effectiveness.get('data_points', 0)}")
                print(f"     Recommendations: {len(effectiveness.get('recommendations', []))}")
    ...
    async def step_6_demonstrate_agent_coordination(self):
        ...
        coordination_result = await self.llm_integration.coordinate_with_other_agents(
            ...
            required_capabilities=["benefits_processing", "tax_processing", "legal_compliance", "citizen_interaction"],
        ...
        print(f"   - Required Capabilities: {coordination_result.get('required_capabilities', [])}")
        print(f"   - Coordination Strategy: {coordination_result.get('coordination_strategy', 'N/A')}")
        print(f"   - Communication Protocol: {coordination_result.get('communication_protocol', 'N/A')}")
        ...
        optimization_recommendations = await self.performance_monitor.optimize_agent_coordination(coordination_data)
        ...
        for i, rec in enumerate(optimization_recommendations[:3], 1):  # Show first 3
            ...
            print(f"      Priority: {rec.priority}, Expected Improvement: {rec.expected_improvement:.1%}")
